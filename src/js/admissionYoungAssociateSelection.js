	...
	*/
		...
		try {
			...
			resOrder.forEach((value, index) => { // 志願列表格式整理
				let add = {
					...
					mainGroup: value.main_group_data.title, // 主學群名稱
					subGroup: value.sub_group_data.title,
					associate_dept: value.title_abbr
				};
			...
			$quotaLinkBtn.attr('href', env.quotaUrl+"/youngassociate.html");
	...
	function _optionalWishTemplating(data) { // 分頁資料渲染（data.length === 0 時不會被呼叫）
		...
		$.each(data, function(index, item){
			...
			<td>
			${item[_showCodeId]} ｜<br>
			${item.mainGroup} - ${item.subGroup} ｜<br>
			${item.school} ｜<br>
			${item.associate_dept} ｜<br>
			${item.engDept} ｜
			</td>
			<td class="text-right">
	...
	function _generateWishList() { // 「渲染已填選志願」
		...
		for(let i in _wishList) {
			...
			<td>
			${_wishList[i][_showCodeId]} ｜<br>
			${_wishList[i].mainGroup} - ${_wishList[i].subGroup} ｜<br>
			${_wishList[i].school} ｜<br>
			${_wishList[i].associate_dept} ｜<br>
			${_wishList[i].engDept} ｜
			</td>
	...
	function _showInfo(){
		...
		const schoolID = _optionalWish[optionalIndex].school_id;
		let quotaUrl = env.quotaUrl;

		$('#modal-title').text(title+"—"+'招生資訊');

		quotaUrl += '/young-associate-detail.html?id='+departmentID+'&school-id='+schoolID+'&tab=nav-shenchaItem';
