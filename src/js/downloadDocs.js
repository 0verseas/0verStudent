(() => {
	...
	*/
	...
	async function _init() {
		...
		.then((data) => {
			personalData=data.student_personal_data;
			...
			.then((data) => {
				...
				if(data.student_qualification_verify.identity === 6) {
					$memo.html(`<ol class="text-danger"><li>請在簡章規定之期限內列印並繳交至國立臺灣師範大學僑先部教務組。</li></ol>`);
				} else { // 非僑先部結業生
					let listHtml = ``;
					...
					if(data.can_admission_selection){
						$memo.html(`<ol class="text-danger">`+listHtml+`</ol>`);
					} else {
						$memo.html(`<ul class="text-danger">`+listHtml+`</ul>`);
					}
				}
			})
			.then(() => {
