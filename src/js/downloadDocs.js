		...
		try{
					...
					if (registrationData.student_qualification_verify.system_id < 3) { // 學士、港二技
						await $('.alert-macau-verification').html(`
							...
							<ul style="list-style-type:none; margin-bottom: 0;">
								...
								<li>
									開放日期：2024 年 11 月 15 日 - 12 月 22 日(12 月 20 日澳門公眾假期休息不開放)
								</li>
								<li>
			...
			if(registrationData.student_qualification_verify.identity === 6) {
				...
				if(registrationData.can_upload_papers){
					...
					listHtml += `<li>如欲放棄上傳部分志願校系審查資料時，可於該志願上傳頁面點選「放棄上傳備審資料」按鍵，惟申請人一旦於系統完成「放棄上傳備審資料」作業並確認提交後，一律不得以任何理由要求撤回或修改，請審慎考量。</li>`;
				}
